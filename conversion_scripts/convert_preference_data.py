...
if args.input_dataset == "nvidia/HelpSteer":
    ...
    for prompt, samples in prompts.items():
        ...
        new_data.append({"chosen": chosen, "rejected": rejected, "source": "helpsteer"})
    ...
    for sample in dataset:
        ...
        new_data.append({"chosen": chosen, "rejected": rejected, "source": "nectar"})
    ...
    for sample in dataset:
        ...
        rejected = sample["rejected"]
        new_data.append(
            {"chosen": chosen, "rejected": rejected, "source": "argilla-capybara"}
        )
    ...
    for sample in dataset:
        ...
        if prompt in prompts_in_argilla:
            ...
            rejected = sample["rejected"]
            new_data.append(
                {"chosen": chosen, "rejected": rejected, "source": "h4-ultrafeedback"}
            )
elif (
    args.input_dataset == "stanfordnlp/SHP" or args.input_dataset == "stanfordnlp/SHP-2"
):
    for el in dataset:
    ...
    for sample in dataset:
        ...
        new_data.append(
            {"chosen": chosen, "rejected": rejected, "source": "orca_dpo_pairs"}
        )
    ...
    for sample in dataset:
        ...
        rejected = [prompt, {"role": "assistant", "content": sample["rejected"]}]
        new_data.append(
            {"chosen": chosen, "rejected": rejected, "source": "orca_dpo_pairs_argilla"}
        )
    ...
    for sample in dataset:
        ...
        multichoice_steps = [
            ...
            if len(step["completions"]) > 1 or step["human_completion"] is not None
        ]
        ...
        for idx, step in enumerate(sample["label"]["steps"]):
            ...
            non_chosen_completions = [
                x["text"] for x in step["completions"] if x["text"] != chosen_completion
            ]
        ...
        new_data.append(
            {"chosen": chosen, "rejected": rejected, "source": "prm800k_phase1"}
        )
    ...
    for prompt, samples in prompts.items():
        ...
        wrong_completions = [
            x for x in samples if x["label"]["finish_reason"] == "found_error"
        ]
        ...
        if chosen[1]["content"] is None or rejected[1]["content"] is None:
            continue
        new_data.append(
            {"chosen": chosen, "rejected": rejected, "source": "prm800k_phase2"}
        )
