...
if __name__ == "__main__":
    ...
    tokens_t = torch.from_numpy(np.array(tokens)).long()
    valid_t = torch.from_numpy(np.array(valid)).long()
    tokens_t = torch.where(valid_t == 1, tokens_t, torch.tensor(-100))
    loss_fct = torch.nn.CrossEntropyLoss()
    loss = loss_fct(logits.view(-1, logits_t.shape[-1]), tokens_t.view(-1))
    print("torch:", loss)
