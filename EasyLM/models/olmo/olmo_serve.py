...
from EasyLM.models.olmo.olmo_model import FlaxOLMoForCausalLM, OLMoConfig
from EasyLM.serving import LMServer
from jax.experimental.pjit import pjit
from jax.sharding import PartitionSpec as PS
from transformers import FlaxLogitsProcessorList, GenerationConfig
...
def main(argv):
    ...
    prefix_tokenizer = OLMoConfig.get_tokenizer(
        FLAGS.tokenizer, truncation_side="left", padding_side="left"
    )
    tokenizer = OLMoConfig.get_tokenizer(
    ...
    model_ps = match_partition_rules(OLMoConfig.get_partition_rules(), params)
    ...
    def forward_loglikelihood(params, rng, batch):
        ...
        # if olmo_config.n_real_tokens is not None:
        #   logits = logits.at[:, :, olmo_config.n_real_tokens:].set(-1e8)
    ...
    class ModelServer(LMServer):
        ...
        def loglikelihood_rolling(text):
            ...
            input_tokens = np.concatenate([bos_tokens, output_tokens[:, :-1]], axis=-1)
            _bos_mask = np.ones((batch_size, 1), dtype=inputs.attention_mask.dtype)
