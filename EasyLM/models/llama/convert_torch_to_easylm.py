...
def main(argv):
    ...
    jax_weights = {
        "transformer": {
            "wte": {
                "embedding": np.concatenate(
                    [
                        ckpt["tok_embeddings.weight"].to(torch.float16).numpy()
                        for ckpt in ckpts
                    ],
                    axis=0,
                )
            },
            "ln_f": {"kernel": ckpts[0]["norm.weight"].to(torch.float16).numpy()},
            "h": {
                "%d" % (layer): {
                    "attention": {
                        "wq": {
        ...
        "lm_head": {
            "kernel": np.concatenate(
                [ckpt["output.weight"].to(torch.float16).numpy() for ckpt in ckpts],
                axis=0,
            ).transpose()
        },
    }
    ...
    pdb.set_trace()
    if FLAGS.streaming:
        StreamingCheckpointer.save_train_state_to_file(jax_weights, FLAGS.output_file)
    else:
