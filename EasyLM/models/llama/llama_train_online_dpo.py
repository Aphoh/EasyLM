...
def dpo_forward_backward(
    ...
    cont_input_ids, cont_attn_mask, _cont_position_ids = (
    ...
    cont_logits = policy_model(
        input_ids, attn_mask, params=policy_params, dropout_rng=rng_generator()
    ...
    cont_logps_pairs = cont_logps.reshape(
        cont_logps.shape[0] // 2, 2, cont_logps.shape[1]
    )
    ...
    # split our logps into pairs
    cont_ref_logps_pairs = cont_ref_logps.reshape(
        cont_ref_logps.shape[0] // 2, 2, cont_ref_logps.shape[1]
    )

    # split our logps into chosen and rejected
    # since they are paired, is just a patter of which idx is chosen
    policy_chosen_logps = jnp.where(
        chosen_mask[:, None], cont_logps_pairs[:, 1], cont_logps_pairs[:, 0]
    )
    policy_rejected_logps = jnp.where(
        chosen_mask[:, None], cont_logps_pairs[:, 0], cont_logps_pairs[:, 1]
    )
    reference_chosen_logps = jnp.where(
        chosen_mask[:, None], cont_ref_logps_pairs[:, 1], cont_ref_logps_pairs[:, 0]
    )
    reference_rejected_logps = jnp.where(
        chosen_mask[:, None], cont_ref_logps_pairs[:, 0], cont_ref_logps_pairs[:, 1]
    )
...
def main(argv):
    ...
    tokenizer = LLaMAConfig.get_tokenizer(
        FLAGS.tokenizer, padding_side="left", truncation_side="left"
    )
    ...
    shard_fns_reward, _ = make_shard_and_gather_fns(
    ...
    def dpo_forward_backward_wrapper(
        ...
        def loss_and_metrics(policy_params):
            return dpo_forward_backward(
        ...
        for epoch in trange(0, FLAGS.num_epochs, ncols=0, position=0):
            for step, batch in zip(
                ...
                if FLAGS.log_freq > 0 and global_step % FLAGS.log_freq == 0:
                    ...
                    stats.update(
                        {
                            "odpo/learning_rate_policy": policy_optimizer_info[
                ...
                if (
                    ...
                    save_checkpoint(
                        policy_train_state, step=global_step, milestone=True
                    )
                elif (
