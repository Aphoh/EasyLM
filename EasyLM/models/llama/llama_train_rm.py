...
def margin_loss_forward(model, params, rng, batch, train=True):
    ...
    if FLAGS.use_margin:
        loss = -jax.nn.log_sigmoid(
            rewards_chosen - rewards_rejected - batch["margin"]
        ).mean()
    else:
...
def main(argv):
    ...
    def train_step(train_state, rng, batch):
        ...
        def loss_and_metrics(params):
            return margin_loss_forward(
                model, params, rng_generator(llama_config.rng_keys()), batch
            )
    ...
    with mesh:
        ...
        if FLAGS.load_checkpoint != "" and FLAGS.load_from_causal_lm:
            ...
            restored_params["params"]["score"] = unfreeze(
                random_init_params["params"]["score"]
            )
            ...
            del (
                causal_train_state_shapes,
                causal_shard_fns,
                causal_train_state_partition,
            )
        ...
        if FLAGS.num_epochs > 0:
            ...
            epoch_counter = trange(
                0, math.ceil(FLAGS.total_steps / steps_per_epoch), ncols=0, position=0
            )
