...
def ppo_loss(
    ...
    new_cont_logits = policy_model(
        input_ids,
        attn_mask,
        params=policy_params["params"],
        dropout_rng=rng_generator(),
    ).logits[:, PL - 1 : -1, :]  # (B, CL, V)
    new_cont_logps = jnp.take_along_axis(
        jax.nn.log_softmax(new_cont_logits, axis=-1),
        cont_input_ids[:, :, None],
        axis=-1,
    ).squeeze(-1)  # (B, CL)
    ...
    pg_losses2 = -advantages * jnp.clip(
        ratio, 1.0 - FLAGS.cliprange, 1.0 + FLAGS.cliprange
    )  # (B, CL)
    pg_loss = masked_mean(jnp.maximum(pg_losses, pg_losses2), cont_attn_mask)

    # run forward pass on value
    new_cont_values = value_model(
        input_ids, attn_mask, params=value_params["params"], dropout_rng=rng_generator()
    ).logits[:, PL - 1 : -1]  # (B, CL)

    new_cont_values_clipped = old_cont_values + jnp.clip(
        new_cont_values - old_cont_values, -FLAGS.cliprange_value, FLAGS.cliprange_value
    )
    ...
    stats = {
        ...
        "ppo/val/error": detach(
            masked_mean(jnp.square(new_cont_values - returns), cont_attn_mask)
        ),
...
def compute_advantages(values, rewards, mask):
    ...
    for t in reversed(range(gen_len)):
        nextvalues = (
            values[:, t + 1] if t < gen_len - 1 else jnp.zeros_like(values[:, t])
        )
...
def ppo_rollout(
    ...
    batch = with_sharding_constraint(
        batch, PS(("dp", "fsdp"))
    )  # dim 0 is sharded across dp and fsdp axes, dim 1 is not sharded

    prompt_input_ids, prompt_attn_mask = (
    ...
    last_token_index = jnp.argmax(
        jnp.cumsum(jnp.where(input_ids == tokenizer.pad_token_id, 0, 1), axis=1), axis=1
    )  # (B)
    input_ids = jnp.concatenate([input_ids, input_ids[:, -1:]], axis=1)  # (B, L+1)
    input_ids = input_ids.at[jnp.arange(input_ids.shape[0]), last_token_index + 1].set(
...
def ppo_forward_backward(
    ...
    reward_prompt_input_ids, reward_prompt_attn_mask = (
        batch["reward_prompt_input_ids"],
        batch["reward_prompt_attn_mask"],
    )
    cont_input_ids, cont_attn_mask, cont_position_ids = (
        batch["cont_input_ids"],
        batch["cont_attn_mask"],
        batch["cont_position_ids"],
    )
    PL = prompt_input_ids.shape[1]

    # run reward model
    reward_input_ids = jnp.concatenate(
        [reward_prompt_input_ids, cont_input_ids], axis=1
    )  # (B, PL+CL)
    reward_attn_mask = jnp.concatenate(
        [reward_prompt_attn_mask, cont_attn_mask], axis=1
    )  # (B, PL+CL)
    reward = reward_model(
        reward_input_ids,
        reward_attn_mask,
    ...
    reward_position_ids = jnp.clip(
        jnp.cumsum(reward_attn_mask, axis=1) - 1, 0, None
    )  # (B, PL+CL)
    reward_last_token_index = jnp.argmax(reward_position_ids, axis=1)  # (B)
    reward_last_token_id = jnp.take_along_axis(
        reward_input_ids, reward_last_token_index[:, None], axis=-1
    ).squeeze(-1)  # (B)
    ...
    # run forward pass on policy
    cont_logits = policy_model(
        input_ids,
        attn_mask,
        params=policy_train_state.params["params"],
        dropout_rng=rng_generator(),
    ).logits[:, PL - 1 : -1, :]  # (B, CL, V)
    cont_logps = jnp.take_along_axis(
        jax.nn.log_softmax(cont_logits, axis=-1), cont_input_ids[:, :, None], axis=-1
    ).squeeze(-1)  # (B, CL)
    cont_logps = jax.lax.stop_gradient(cont_logps)

    # run forward pass on reference
    cont_ref_logits = reference_model(
        input_ids,
        attn_mask,
        params=reference_params["params"],
        dropout_rng=rng_generator(),
    ).logits[:, PL - 1 : -1, :]  # (B, CL, V)
    cont_ref_logps = jnp.take_along_axis(
        jax.nn.log_softmax(cont_ref_logits, axis=-1),
        cont_input_ids[:, :, None],
        axis=-1,
    ).squeeze(-1)  # (B, CL)
    ...
    # run forward pass on value
    cont_values = value_model(
        input_ids,
        attn_mask,
        params=value_train_state.params["params"],
        dropout_rng=rng_generator(),
    ).logits[:, PL - 1 : -1]  # (B, CL)
    ...
    rewards = non_score_rewards.at[
        jnp.arange(input_ids.shape[0]), cont_last_token_index
    ].add(score)  # (B, CL)
    rewards = jax.lax.stop_gradient(rewards)

    # compute advantages
    advantages, returns = compute_advantages(
        cont_values, rewards, cont_attn_mask
    )  # (B, CL), (B, CL)
    ...
    stats = {
        ...
        "ppo/mean_score_total": detach(
            jnp.mean(masked_sum(rewards, cont_attn_mask, axis=1))
        ),
        "ppo/mean_non_score_reward": detach(
            masked_mean(non_score_rewards, cont_attn_mask)
        ),
        "ppo/mean_non_score_reward_sum": detach(
            jnp.mean(masked_sum(non_score_rewards, cont_attn_mask, axis=1))
        ),
    ...
    (_, new_stats), (policy_grads, value_grads) = grad_fn(
        policy_train_state.params, value_train_state.params
    )
...
def main(argv):
    ...
    tokenizer = LlamaTokenizerFast.from_pretrained(
        FLAGS.tokenizer, use_auth_token=os.getenv("HF_TOKEN", None), padding_side="left"
    )
    ...
    wrapped_dataset = (
        dataset.dataset if isinstance(dataset, torch.utils.data.DataLoader) else dataset
    )
    ...
    steps_per_epoch = (
        ...
        else min(steps_per_epoch, FLAGS.max_steps_per_epoch)
    )
    total_steps = FLAGS.num_epochs * steps_per_epoch
    completion_batch_size = prompt_batch_size * FLAGS.rollouts_per_prompt
    if (
        FLAGS.mini_batch_size * FLAGS.optimizer.accumulate_gradient_steps
    ...
    policy_freeze_grad_updates = math.ceil(
        FLAGS.policy_freeze_epochs * grad_updates_per_epoch
    )
    ...
    policy_model = FlaxLLaMAForCausalLM(
        llama_config_policy, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False
    )
    value_model = FlaxLLaMAForTokenRegression(
        llama_config_reward, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False
    )
    ...
    value_optimizer, value_optimizer_info = OptimizerFactory.get_optimizer(
        FLAGS.optimizer
    )
    FLAGS.optimizer.adamw_optimizer.lr_freeze_steps = policy_freeze_grad_updates
    policy_optimizer, policy_optimizer_info = OptimizerFactory.get_optimizer(
        FLAGS.optimizer
    )
    ...
    train_state_shapes_policy = jax.eval_shape(
        init_fn_policy, next_rng()
    )  # .params = {'params': {'transformer', 'lm_head'}} => .params = {'transformer', 'lm_head'}
    train_state_partition_policy = match_partition_rules(
        LLaMAConfig.get_partition_rules(), train_state_shapes_policy
    )
    shard_fns_policy, gather_fns_policy = make_shard_and_gather_fns(
        train_state_partition_policy, train_state_shapes_policy
    )
    ...
    train_state_shapes_reward = jax.eval_shape(
        init_fn_reward, next_rng()
    )  # .params = {'params': {'transformer', 'lm_head'}} => .params = {'transformer', 'lm_head'}
    train_state_partition_reward = match_partition_rules(
        LLaMAConfig.get_partition_rules(), train_state_shapes_reward
    )
    shard_fns_reward, gather_fns_reward = make_shard_and_gather_fns(
        train_state_partition_reward, train_state_shapes_reward
    )
    ...
    sharded_ppo_forward_backward = pjit(
        ...
        in_shardings=(
            train_state_partition_policy,
            train_state_partition_policy.params,
            train_state_partition_reward,
            train_state_partition_reward.params,
            PS(),
            PS(),
        ),
        out_shardings=(
    ...
    with mesh:
        ...
        if policy_train_state is None:
            if policy_params is None:
                ...
                policy_train_state = sharded_create_trainstate_from_params_policy(
                    policy_params
                )
                del policy_params
        ...
        if FLAGS.load_checkpoint_reward != "":
            ...
            value_train_state, value_params = checkpointer.load_trainstate_checkpoint(
                FLAGS.load_checkpoint_reward,
                train_state_shapes_reward,
                shard_fns_reward,
            )
        ...
        if value_train_state is None:
            if value_params is None:
                ...
                value_train_state = sharded_create_trainstate_from_params_reward(
                    value_params
                )
                del value_params
        ...
        if FLAGS.load_checkpoint_policy != "":
            ...
            (
                ...
                reference_params,
            ) = checkpointer.load_trainstate_checkpoint(
                FLAGS.load_checkpoint_policy,
                train_state_shapes_policy,
                shard_fns_policy,
            )
        ...
        if FLAGS.load_checkpoint_reward != "":
            ...
            reward_train_state, reward_params = checkpointer.load_trainstate_checkpoint(
                FLAGS.load_checkpoint_reward,
                train_state_shapes_reward,
                shard_fns_reward,
            )
        ...
        for epoch in trange(0, FLAGS.num_epochs, ncols=0, position=0):
            for step, batch in zip(
                trange(0, steps_per_epoch, ncols=0, position=1), dataset
            ):
                global_step += 1
                ...
                batch = {
                    k: jnp.repeat(v, FLAGS.rollouts_per_prompt, axis=0)
                    for k, v in batch.items()
                }

                t0 = time.time()
                t = time.time()
                sharded_rng, batch = sharded_ppo_rollout(
                    policy_train_state, sharded_rng, batch
                )
                ...
                if FLAGS.generate_only:
                    ...
                    stats["game_log"] = wandb.Table(
                        columns=["query", "response"], rows=rows
                    )
                    logger.log(stats)
                    continue
                ...
                for ppo_epoch in range(FLAGS.ppo_epochs):
                    for mb_start in range(
                        0, batch["input_ids"].shape[0], FLAGS.mini_batch_size
                    ):
                        ...
                        jax.tree_map(
                            lambda x: x.block_until_ready(), policy_train_state.params
                        )
                        jax.tree_map(
                            lambda x: x.block_until_ready(),
                            policy_train_state.opt_state,
                        )
                        jax.tree_map(
                            lambda x: x.block_until_ready(), value_train_state.params
                        )
                        jax.tree_map(
                            lambda x: x.block_until_ready(), value_train_state.opt_state
                        )
                ...
                batch = {
                    k: jnp.concatenate([mb[k] for mb in all_mbs], axis=0)
                    for k in all_mbs[0].keys()
                }
                stats = {
                    k: jnp.mean(jnp.stack([s[k] for s in all_stats], axis=0), axis=0)
                ...
                print(
                    f"step={global_step}, time_rollout={time_rollout:.2f},"
                    f" time_forward_backward={time_forward_backward:.2f}, time_total={time_total:.2f}"
                )
                ...
                if FLAGS.log_freq > 0 and global_step % FLAGS.log_freq == 0:
                    ...
                    stats.update(
                        {
                            "ppo/learning_rate_policy": policy_optimizer_info[
                                "learning_rate_schedule"
                            ](global_step * grad_updates_per_step).item(),
                            "ppo/learning_rate_value": value_optimizer_info[
                                "learning_rate_schedule"
                            ](global_step * grad_updates_per_step).item(),
                            "time/ppo/rollout": time_rollout,
                    ...
                    rows = [
                        [q, r, float(reward)]
                        for q, r, reward in zip(queries, responses, rewards)
                    ]
                    stats["game_log"] = wandb.Table(
                        columns=["query", "response", "reward"], rows=rows
                    )
                    logger.log(stats)

                if (
                    FLAGS.save_milestone_freq > 0
                    and global_step % FLAGS.save_milestone_freq == 0
                ):
                    save_checkpoint(
                        policy_train_state,
                        value_train_state,
                    ...
                    and global_step % FLAGS.save_model_freq == 0
                ):
                    save_checkpoint(
                        policy_train_state, value_train_state, step=global_step
                    )
