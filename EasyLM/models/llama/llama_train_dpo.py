...
def concatenated_forward(model, params, rng, batch, train=True):
    ...
    concat_input_ids = jnp.concatenate(
        [batch["chosen_input_ids"], batch["rejected_input_ids"]], axis=0
    )
    concat_attn_mask = jnp.concatenate(
        [batch["chosen_attn_mask"], batch["rejected_attn_mask"]], axis=0
    )
    logits = model.apply(
        params,
        concat_input_ids,
    ...
    chosen_logps = convert_logits_to_logps(
        chosen_logits,
        batch["chosen_input_ids"],
        batch["chosen_loss_mask"],
        model.config.pad_token_id,
    )
    rejected_logps = convert_logits_to_logps(
        rejected_logits,
...
def dpo_forward(
    ...
    assert (
        reference_logps is not None or reference_params is not None
    ), "Must provide either reference logps or reference params!"
    if reference_logps is not None:
        ...
        reference_chosen_logps, reference_rejected_logps = concatenated_forward(
            model, reference_params, rng, batch
        )
...
def main(argv):
    ...
    def train_step(
        train_state, rng, batch, reference_logps=None, reference_train_state=None
    ):
        rng_generator = JaxRNG(rng)
        ...
        def loss_and_metrics(params):
            return dpo_forward(
                ...
                if reference_train_state is not None
                else None,
            )
    ...
    def no_model_concate_forward(train_state, rng, batch):
        return concatenated_forward(model, train_state, rng, batch, train=False)
    ...
    with mesh:
        train_state, restored_params, reference_train_state, reference_params = (
            None,
            None,
            None,
            None,
        ...
        if not FLAGS.precalculate_reference_logps:
            ...
            if FLAGS.load_checkpoint != "":
                ...
                print(
                    "Warning, your dpo reference params are not loaded from a checkpoint!"
                )
            ...
            if reference_train_state is None and reference_params is None:
                ...
                # Restore from params but initialize train_state
                reference_train_state = sharded_create_trainstate_from_params(
                    reference_params
                )
            ...
            for batch in tqdm(no_shuffle_dataset):
                ...
                (
                    reference_chosen_logps,
                    reference_rejected_logps,
                ) = sharded_concatenated_forward(
                    train_state.params, rng_generator(llama_config.rng_keys()), batch
                )
                all_reference_chosen_logps.append(
                    jax.device_get(reference_chosen_logps)
                )
                all_reference_rejected_logps.append(
                    jax.device_get(reference_rejected_logps)
                )
                del reference_chosen_logps, reference_rejected_logps
            all_reference_chosen_logps = jnp.concatenate(
                all_reference_chosen_logps, axis=0
            )
            all_reference_rejected_logps = jnp.concatenate(
                all_reference_rejected_logps, axis=0
            )
