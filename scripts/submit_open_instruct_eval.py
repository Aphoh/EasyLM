...
parser.add_argument(
    "--beaker_image", type=str, default=None, help="If given, use this Beaker image."
)
...
parser.add_argument(
    "--gsm_stop_at_double_newline",
    action="store_true",
    help="If given, stop generation at double newline for GSM8k.",
)
...
d = copy.deepcopy(d1)
model_name = (
    model_info[0] + f"_{model_info[2]}" if model_info[2] is not None else model_info[0]
)
...
for experiment_group in experiment_groups:
    ...
    if model_info[0].startswith(
        "hf-"
    ):  # if it's a huggingface model, load it from the model hub
        task_spec["arguments"] = [
        ...
        task_spec["arguments"] = [
            task_spec["arguments"][0].replace(
                "--tokenizer_name_or_path /model",
                "--model_name_or_path " + model_info[1],
            )
        ]
    elif model_info[1].startswith(
        "/"
    ):  # if it's a local model, load it from the local directory
        task_spec["arguments"] = [
            task_spec["arguments"][0].replace(
                "--model_name_or_path /model", "--model_name_or_path " + model_info[1]
            )
    ...
    if model_info[2] is not None:
        ...
        search = re.search("--model_name_or_path (\S+)", task_spec["arguments"][0])
        if search is None:
            raise ValueError(
                "Model name or path not found in task arguments: {}".format(
                    task_spec["arguments"][0]
                )
            )
        model_name_or_path = search.group(1)
        # replace the model path with the checkpoint subfolder.
        task_spec["arguments"] = [
            task_spec["arguments"][0].replace(
                model_name_or_path,
                model_name_or_path + "/" + model_info[2],
                1,  # type: ignore
            )
        ]
    ...
    if model_info[3] == "vanilla_lm":
        task_spec["arguments"] = [
            task_spec["arguments"][0].replace("--use_chat_format", "")
        ]
    ...
    if "13B" in model_info[0]:
        ...
        if "--eval_batch_size" in task_spec["arguments"][0]:
            original_batch_size = re.search(
                "--eval_batch_size (\d+)", task_spec["arguments"][0]
            )
            if original_batch_size is None:
                raise ValueError("Batch size not found in task arguments")
            original_batch_size = original_batch_size.group(1)  # type: ignore
            new_batch_size = max(1, int(original_batch_size) // 2)  # type: ignore
            task_spec["arguments"] = [
                task_spec["arguments"][0].replace(
    ...
    if "30B" in model_info[0] or "34B" in model_info[0]:
        ...
        if "--eval_batch_size" in task_spec["arguments"][0]:
            ...
            if original_batch_size is None:
                raise ValueError("Batch size not found in task arguments")
            original_batch_size = original_batch_size.group(1)  # type: ignore
            new_batch_size = max(1, int(original_batch_size) // 4)  # type: ignore
            task_spec["arguments"] = [
                task_spec["arguments"][0].replace(
                    "--eval_batch_size {}".format(original_batch_size),
                    "--eval_batch_size {}".format(new_batch_size),
        ...
        if "--eval_batch_size" in task_spec["arguments"][0]:
            ...
            if original_batch_size is not None:
                raise ValueError("Original batch size not found")
            original_batch_size = original_batch_size.group(1)  # type: ignore
            new_batch_size = max(1, int(original_batch_size) // 4)  # type: ignore
            task_spec["arguments"] = [
                task_spec["arguments"][0].replace(
    ...
    if "llama2-chat" in model_info[0]:
        ...
        if "--use_vllm" in task_spec["arguments"][0]:
            ...
            task_spec["arguments"] = [
                task_spec["arguments"][0].replace("--use_vllm", "")
            ]
